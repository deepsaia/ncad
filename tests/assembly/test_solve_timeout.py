"""build_timeout_s: the build/solve wall-clock bound reads NCAD_BUILD_TIMEOUT, safe default."""

from ncad.build.build_timeout import _DEFAULT_BUILD_TIMEOUT_S, build_timeout_s


def test_default_when_unset(monkeypatch):
    monkeypatch.delenv("NCAD_BUILD_TIMEOUT", raising=False)
    assert build_timeout_s() == _DEFAULT_BUILD_TIMEOUT_S


def test_env_override(monkeypatch):
    monkeypatch.setenv("NCAD_BUILD_TIMEOUT", "300")
    assert build_timeout_s() == 300.0
...
def test_default_is_generous():
    ...
    assert _DEFAULT_BUILD_TIMEOUT_S >= 600.0
