...
def test_below_endurance_limit_is_infinite_life():
    # Fully reversed (R=-1), peak 200e6 -> alternating 200e6 < endurance 220e6 -> infinite.
    out = FatigueCalculator().life(200e6, -1.0, _STEEL)
    assert out["infinite_life"] is True
    assert out["cycles_to_failure"] is None
    assert out["alternating_stress"] == pytest.approx(200e6)
    assert out["mean_stress"] == pytest.approx(0.0)


def test_above_endurance_limit_gives_finite_basquin_life():
    # Fully reversed peak 600e6 -> alternating 600e6 > endurance
    # -> finite N = (600e6/1200e6)^(1/-0.085).
    out = FatigueCalculator().life(600e6, -1.0, _STEEL)
    assert out["infinite_life"] is False
    expected = (600e6 / 1200e6) ** (1.0 / -0.085)
...
def test_safety_factor_is_endurance_over_alternating():
    out = FatigueCalculator().life(200e6, -1.0, _STEEL)
    assert out["fatigue_safety_factor"] == pytest.approx(220e6 / 200e6, rel=1e-6)
