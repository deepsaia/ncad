...
# Part documents only: an assembly document (.asm.hocon) and a motion study (.motion.hocon) are
# different schema/build paths (ncad assemble / ncad motion), covered by their own gate tests, not
# this part-build sweep.
_EXAMPLE_DOCS = sorted(p for p in _EXAMPLES_DIR.glob("gate-*/*.hocon")
                       if not (p.name.endswith(".asm.hocon") or p.name.endswith(".motion.hocon")))
