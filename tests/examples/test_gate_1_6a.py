"""End-to-end coverage for bucket 1.6a ellipse + arc_polar sketch entities.

Builds each gate example on the real kernel (slow), round-trips it to STEP, and checks
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-1.6a"
_GOLDEN = Path(__file__).resolve().parents[1] / "build" / "golden"


@pytest.mark.parametrize("name", ["elliptical_flange", "luggage_tag"])
def test_gate_1_6a_builds_and_step_round_trips(name, tmp_path):
...
@pytest.mark.parametrize("name", ["elliptical_flange", "luggage_tag"])
def test_gate_1_6a_is_deterministic(name):
...
@pytest.mark.parametrize("name", ["elliptical_flange", "luggage_tag"])
def test_gate_1_6a_signature_matches_golden(name):
    from ncad.build.equality_comparator import EqualityComparator
