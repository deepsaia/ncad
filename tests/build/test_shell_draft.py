"""End-to-end coverage for bucket 2.7 shell + draft.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.7"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["shelled_box", "drafted_boss"])
def test_shell_draft_example_builds_and_step_round_trips(name, tmp_path):
