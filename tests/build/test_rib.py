"""End-to-end coverage for bucket 2.5 rib / web / stiffener.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.5"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["t_rib", "curved_rib"])
def test_rib_example_builds_and_step_round_trips(name, tmp_path):
