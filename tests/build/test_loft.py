"""End-to-end coverage for bucket 2.4 loft.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.4"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["lofted_transition", "lofted_cone", "ruled_loft"])
def test_loft_example_builds_and_step_round_trips(name, tmp_path):
