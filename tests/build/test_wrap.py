"""End-to-end coverage for bucket 2.8b wrap.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.8b"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["engraved_nameplate", "embossed_logo"])
def test_wrap_example_builds_and_step_round_trips(name, tmp_path):
