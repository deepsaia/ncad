"""End-to-end coverage for bucket 2.8 hole wizard.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.8"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["counterbore_plate", "countersink_plate", "tapped_boss"])
def test_hole_wizard_example_builds_and_step_round_trips(name, tmp_path):
