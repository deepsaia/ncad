"""End-to-end coverage for bucket 2.6 chamfer variants.
...
_DIR = Path(__file__).resolve().parents[2] / "examples" / "gate-2.6"


@pytest.mark.slow
@pytest.mark.parametrize("name", ["two_distance_chamfer", "angle_chamfer"])
def test_chamfer_variant_example_builds_and_step_round_trips(name, tmp_path):
