...
ncad is driven by authored text documents. There are seven kinds. Three of them (motion, physics,
analysis) are **overlays**: they reference another document and add only the semantics it does not
carry, so nothing is authored twice.
...
mesh} }`; inertials are computed from geometry. See [authoring robots](../guides/authoring-robots.md).
...
motion study `<name>.motion.json`; a robot the `.urdf`/`.xml`/`.sdf` artifact plus `<name>.robot.json`;
an analysis `<name>.analysis.json` plus a field mesh.
